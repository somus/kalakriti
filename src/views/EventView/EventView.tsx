...
import { ScanQrCodeIcon, SheetIcon, UploadCloudIcon } from 'lucide-react';
...
import ScoresheetPhotoFormDialog from '../CenterEventView/ScoresheetPhotoFormDialog';
...
function eventQuery(z: Zero, eventId: string) {
	return z.query.subEvents
		...
		.related('event', q =>
			q.related('coordinators').related('category').related('subEvents')
		)
		.one();
}
										...
										<DropdownMenu key='scoresheet'>
											...
											>
												<Dialog>
													...
													>
														<div className='relative h-[calc(100vh-220px)] w-full overflow-clip rounded-md bg-transparent'>
															<img
																src={`${import.meta.env.DEV ? 'https://kalakriti.proudindian.ngo' : ''}/cdn-cgi/image/height=800,quality=75/${env.VITE_IMAGE_CDN}/${subEvent.scoreSheetPath}`}
												...
												<DropdownMenuItem
													variant='destructive'
													onSelect={() => {
														zero.mutate.subEvents
															.updateScoresheetPhoto({
																id: subEvent.id,
																scoresheetPhoto: null
															})
															.client.then(async () => {
																// Delete photo from R2 bucket
																const token = await getToken();
																await fetch(
																	`${env.VITE_API_SERVER}/deleteAsset`,
																	{
																		method: 'DELETE',
																		headers: {
																		...
																		body: JSON.stringify({
																			filePath: subEvent.scoreSheetPath
																		})
																	}
																);
															})
															.catch((e: Error) => {
																toast.error('Error deleting scoresheet photo', {
																	description:
