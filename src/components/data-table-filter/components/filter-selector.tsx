...
export const FilterSelector = memo(
	NonMemoFilterSelector
) as typeof NonMemoFilterSelector;

function NonMemoFilterSelector<TData>({
	filters,
	columns,
	actions,
	strategy,
...
export function FilterableColumn<TData, TType extends ColumnDataType, TVal>({
	...
	const prefetch = useCallback(() => {
		column
			.prefetchOptions()
			.catch(e => console.log('Prefetch options failed', e));
		column
			.prefetchValues()
			.catch(e => console.log('Prefetch values failed', e));
		column
			.prefetchFacetedUniqueValues()
			.catch(e => console.log('Prefetch faceted unique values failed', e));
		column
			.prefetchFacetedMinMaxValues()
			.catch(e => console.log('Prefetch faceted min max values failed', e));
	}, [column]);
...
export const QuickSearchFilters = memo(
	NonMemoQuickSearchFilters
) as typeof NonMemoQuickSearchFilters;

function NonMemoQuickSearchFilters<TData>({
	search,
	filters,
	columns,
	actions
	...
	const cols = useMemo(
		() =>
			columns.filter(c =>
				isAnyOf<ColumnDataType>(c.type as ColumnDataType, [
					'option',
					'multiOption'
				])
			),
